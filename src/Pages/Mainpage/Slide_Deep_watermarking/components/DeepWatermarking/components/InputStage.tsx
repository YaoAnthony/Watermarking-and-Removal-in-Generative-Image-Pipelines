...
export const InputStage: React.FC<InputStageProps> = ({ image, onImageUpload, payload, setPayload }) => {
  ...
  return (
    <div className="flex flex-col h-full space-y-4">
      {/* Image Uploader */}
      <div className="group relative w-full aspect-square bg-slate-50 rounded-lg border-2 border-dashed border-slate-300 hover:border-[#8D9F87] transition-colors cursor-pointer overflow-hidden" onClick={handleBoxClick}>
        <input 
          type="file" 
          ref={fileInputRef} 
          className="hidden" 
        ...
        {image ? (
          ...
          <div className="flex flex-col items-center justify-center h-full text-slate-400">
            <Upload className="w-10 h-10 mb-2 group-hover:text-[#8D9F87] transition-colors" />
      ...
      <div className="mt-auto">
        ...
        <div className="relative">
          <input
            ...
            className="w-full bg-slate-50 border border-slate-200 rounded-md py-2 px-3 text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-[#8D9F87]/50 font-mono"
            placeholder="Enter secret..."
          />
          <div className="flex space-x-1 mt-2">
             {/* Binary visualization dots */}
             {Array.from({ length: 24 }).map((_, i) => (
               <div key={i} className={`w-1 h-1 rounded-full ${i < payload.length * 2 ? 'bg-[#5F6F52]' : 'bg-slate-200'}`} />
             ))}
          </div>
